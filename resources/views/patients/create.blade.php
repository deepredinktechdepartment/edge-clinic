    ...
    <input type="number" 
           ...
           max="120">
                    ...
                    <div class="gender-group mb-3">
                        <div class="d-flex gap-3 mt-1">
                            <div class="form-check">
                                <input class="form-check-input @error('gender') is-invalid @enderror" type="radio" name="gender" value="M" {{ old('gender', $patient->gender ?? '') == 'M' ? 'checked' : '' }}>
                                <label class="form-check-label">Male</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input @error('gender') is-invalid @enderror" type="radio" name="gender" value="F" {{ old('gender', $patient->gender ?? '') == 'F' ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="bookingfor-group">
                            <div class="d-flex flex-wrap gap-3 mt-1">
                                ...
                                @foreach($bfOptions as $opt)
                                    <div class="form-check">
                                        <input class="form-check-input bookingfor" type="radio" name="bookingfor" value="{{ $opt }}"
                        ...
                        <input type="text" name="other_reason" id="other_reason" class="form-control mt-2" placeholder="Specify other" value="{{ $patient->other_reason ?? '' }}" style="{{ (isset($patient) && $patient->bookingfor == 'Others') ? '' : 'display:none;' }}">
                    </div>
...
$(document).ready(function(){
    ...
    function updateHiddenPhoneFields() {
        ...
        if (!e164) return;
        var phoneNumber = e164.replace('+' + dialCode, '').replace(/\D/g, '').replace(/^0+/, '');
    ...
    function fetchPatientsByPhone() {
        ...
        $.get("{{ url('/patients/by-phone') }}", {country_code: countryCode, phone: phone})
            .done(function(res){
                // Always show modal
                if(res.count >= 4 && !$('input[name="id"]').val()) {
                    alert("Maximum 4 patients already exist with this phone number. You can select an existing patient.");
                    //$('#submitBtn').prop('disabled', true);
                } else {
    ...
    function renderPatientPicker(patients){
        ...
        patients.forEach(p => {
            let item = $(`<button type="button" class="list-group-item list-group-item-action"><strong>${p.name}</strong> <small class="text-muted ms-2">Age: ${p.age}</small></button>`);
    ...
    // jQuery validation
    $.validator.addMethod("phoneIntl", function(){return iti.isValidNumber();},"Please enter a valid phone number");
    ...
    $('#patient-form').validate({
        ...
        rules: {
            ...
            email: {email:true},
            age: {required:true, digits:true, min:0, max:100},
            gender: {required:true},
            bookingfor: {required:true},
            other_reason: {required:function(){return $('input[name="bookingfor"]:checked').val() === 'Others';}, minlength:2}
        },
        messages: {
            phone: {required:"Please enter phone number"},
            name: {required:"Please enter name", minlength:"Minimum 2 characters"},
            age: {required:"Please enter age"},
            gender: {required:"Please select gender"},
            bookingfor: {required:"Please select booking for"}
        },
        errorElement: 'div',
        errorClass: 'text-danger mt-1',
        errorPlacement: function(error, element){
            if(element.attr("name")==="gender"){error.appendTo($('.gender-group'));}
            else if(element.attr("name")==="bookingfor"){error.appendTo($('.bookingfor-group'));}
            else{error.insertAfter(element);}
        },
        highlight: function(element){
            if($(element).attr('type')==='radio'){ $(element).closest('.mb-3').addClass('has-error'); }
            else { $(element).addClass('is-invalid'); }
        },
        unhighlight: function(element){
            if($(element).attr('type')==='radio'){ $(element).closest('.mb-3').removeClass('has-error'); }
