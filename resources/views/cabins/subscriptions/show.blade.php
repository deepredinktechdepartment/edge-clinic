...
<div class="cabin-shell">
    ...
    @php $activeDays = array_map('intval', $subscription->subscription_days ?: [0,1,2,3,4,5,6]); @endphp
    <div class="row g-3">
        <div class="col-lg-8"><div class="cabin-panel h-100"><div class="panel-body"><div class="d-flex gap-3 align-items-center"><div class="cabin-avatar">{{ $doctorInitials ?: 'DR' }}</div><div><div class="mini-label">Doctor subscription</div><div class="fw-bold text-dark fs-5">{{ $doctorName }}</div><div class="text-muted">{{ $subscription->cabin->cabin_code ?? '-' }} · {{ $subscription->cabin->name ?? '-' }}</div></div><span class="badge {{ $statusClass }} ms-auto">{{ ucfirst($subscription->status) }}</span></div></div></div></div>
        <div class="col-lg-4"><div class="cabin-panel h-100"><div class="panel-body"><div class="mini-label">Monthly total</div><div class="fw-bold fs-4 text-dark">Rs {{ number_format((float) $subscription->total_amount, 2) }}</div><div class="text-muted small">Invoice day: {{ $subscription->invoice_day ?: '-' }}</div></div></div></div>
    </div>
    <div class="cabin-panel"><div class="panel-head"><div><h5 class="mb-1">Schedule</h5><div class="text-muted">Only highlighted days and this time window are reserved.</div></div></div><div class="panel-body"><div class="d-flex flex-wrap gap-4 align-items-center"><div><div class="mini-label">Period</div><strong>{{ optional($subscription->start_date)->format('d M Y') }} – {{ optional($subscription->end_date)->format('d M Y') }}</strong></div><div><div class="mini-label">Daily timing</div><strong>{{ substr($subscription->subscription_start_time ?: '', 0, 5) }} – {{ substr($subscription->subscription_end_time ?: '', 0, 5) }}</strong></div><div class="cabin-subscription-days">@foreach(['S','M','T','W','T','F','S'] as $number => $day)<span class="{{ in_array($number, $activeDays, true) ? 'active' : '' }}">{{ $day }}</span>@endforeach</div></div></div></div>
    <div class="cabin-panel"><div class="panel-head"><div><h5 class="mb-1">Invoices</h5><div class="text-muted">Monthly invoices linked to this doctor, cabin, and subscription period.</div></div><a href="{{ route('admin.cabins.invoices.create', ['doctor_id' => $subscription->doctor_id, 'cabin_id' => $subscription->cabin_id, 'billing_type' => 'monthly']) }}" class="btn btn-outline-secondary btn-sm">Create Invoice</a></div><div class="panel-body">@forelse($invoices as $invoice)<div class="d-flex justify-content-between align-items-center gap-3 border-bottom py-3"><div><strong class="text-dark">{{ $invoice->invoice_number }}</strong><div class="text-muted small">{{ optional($invoice->invoice_date)->format('d M Y') }} · {{ optional($invoice->period_start)->format('d M Y') }} – {{ optional($invoice->period_end)->format('d M Y') }}</div></div><div class="text-end"><div class="fw-semibold text-dark">Rs {{ number_format((float) $invoice->total_amount, 2) }}</div><span class="badge text-bg-secondary">{{ ucfirst($invoice->status) }}</span><a href="{{ route('admin.cabins.invoices.show', $invoice->id) }}" class="btn btn-outline-secondary btn-sm ms-2">View</a></div></div>@empty<div class="empty-note">No invoice has been created for this subscription yet.</div>@endforelse</div></div>
