...
<div class="cabin-shell">
    ...
    <div class="invoice-screen-sheet">
        ...
        <div class="invoice-screen-parties"><div><div class="mini-label">Billed To</div><strong>{{ $invoice->doctor->name ?? '-' }}</strong>@if($invoice->doctor?->designation)<div class="text-muted">{{ $invoice->doctor->designation }}</div>@endif @if($invoice->doctor?->qualification)<div class="text-muted">{{ $invoice->doctor->qualification }}</div>@endif @if($invoice->doctor?->registration_number)<div class="text-muted">Dr. Reg. No: {{ $invoice->doctor->registration_number }}</div>@endif <div class="text-muted">Cabin: {{ $invoice->cabin->cabin_code ?? 'Multiple cabins' }} {{ $invoice->cabin->name ? '· ' . $invoice->cabin->name : '' }}</div></div><div><div class="mini-label">Invoice Details</div><div>Invoice Date: <strong>{{ optional($invoice->invoice_date)->format('d M Y') }}</strong></div><div>Due Date: <strong>{{ optional($invoice->due_date)->format('d M Y') }}</strong></div><div>Billing {{ optional($invoice->period_start)->isSameDay($invoice->period_end) ? 'Date' : 'Period' }}: <strong>{{ optional($invoice->period_start)->format('d M Y') }}@if(!optional($invoice->period_start)->isSameDay($invoice->period_end)) – {{ optional($invoice->period_start)->format('d M Y') }}@endif</strong></div></div></div>
